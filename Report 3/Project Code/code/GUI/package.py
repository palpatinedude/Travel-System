...
def packagesWindow(role, user_id):
    ...
    if role == "Service Provider":
        membership_options = [("Premium", {"Monthly": "14.99", "6-monthly": "10.00", "Yearly": "8.00"})]
    elif role == "Simple User":
        membership_options = [("Basic", {"Monthly": "12.99", "6-monthly": "8.00", "Yearly": "6.00"})]
    elif role == "Business Partner":
        membership_options = [("Professional", {"Monthly": "14.99", "6-monthly": "10.00", "Yearly": "8.00"})]

    memberships_frame = tk.Frame(packages_window, bg="#1e272e")
    memberships_frame.pack(pady=6)

    membership_colors = {"Monthly": "#fbc531", "6-monthly": "#e74c3c", "Yearly": "#3498db"}

    # dreate rectangles representing membership packages
    for i, (membership, details) in enumerate(membership_options):
        for j, (duration, price) in enumerate(details.items()):
            # dreate a frame for each membership option
            membership_frame = tk.Frame(memberships_frame, bd=2, relief="solid", bg=membership_colors[duration])
            membership_frame.grid(row=j, column=i, padx=20, pady=20, ipadx=55, ipady=23)
